...
Q2 (customer-category growth) and Q5 (revenue by territory).
"""
...
from dashboard.queries import (
    revenue_by_month, revenue_by_category_year, top_products, revenue_by_territory,
)
from dashboard.charts import line_revenue, area_category_year, bar_top
...
# ---- Top products + Revenue by territory (side by side) ---------------------
...
with left:
    ...
    st.plotly_chart(
        bar_top(df_top, x="product", y="revenue", color=None,
...
with right:
    st.subheader("Receita por território de vendas")
    df_terr = revenue_by_territory(filters["year_range"], filters["categories"], filters["territories"])
    st.plotly_chart(
        bar_top(df_terr, x="salesterritory", y="revenue", color=None,
